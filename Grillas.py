...
class GrillaValores(Grilla):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [Qt.Key_Return, Qt.Key_Enter, Qt.Key_Tab] and col == 1:
            ...
            if codigo:
                ...
                if codigo == '1':
                    ...
                    self.columnasHabilitadas = [0, 1, 3, 4, 5, 6, 7, 8, 9, 10]
                    self.HabilitaColumnas(fila=row)
        ...
        if event.key() == Qt.Key_F2 and col == 4:
            _ventana = Banco().Busqueda()
            if _ventana.lRetval:
                self.ModificaItem(valor=_ventana.ValorRetorno,
                                  fila=row, col=4)
        super(GrillaValores, self).keyPressEvent(event)
        self.keyPressed.emit(event.key(),self.shift, self.ctrl)
    ...
    def CreaCabecera(self):
        ...
        self.enabled = True
        cabecera = [
            'Caja', 'Valor', 'Detalle', 'Importe', 'Banco', 'Sucursal', 'Numero', 'Vencimiento', 'CUIT', 'Dato',
            'Cta Ban', 'CP', 'id'
        ]
    ...
    def SumaTodo(self, col=''):
        nTotal = 0.
        for x in range(self.rowCount()):
            pago = self.ObtenerItem(fila=x, col='Importe') or 0
            nTotal += float(pago)
